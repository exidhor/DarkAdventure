                          ...
                          unsigned numberOfGraphicsComponents)
{
	...
	m_components.construct(numberOfGraphicsComponents);
}

GraphicsComponentID GraphicsEngine::getComponent()
{
	unsigned freeID = m_components.getAvailableID();

	return GraphicsComponentID(freeID);
}

void GraphicsEngine::freeComponent(GraphicsComponentID id)
{
	m_components.free(id.getValue());
}
